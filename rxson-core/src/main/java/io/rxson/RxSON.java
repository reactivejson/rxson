...
import io.rxson.rxrest.NoReactiveProviderException;
...
public final class RxSON {
    ...
    private final ReactiveProvider provider;

    //TODO, cancel support

    RxSON(final HttpClient client, final ReactiveProvider provider, final JsonSurfer surfer, final long n) {
        this.client = client;
    ...
    public <T> T create(final Class<T> clazz, final HttpRequest req) {
        final var subscriber = new ReactiveSubscriber<>(clazz, provider, surfer);
        subscriber.setN(n);
     ...
     * <p>
     * Reactor
     * {@code Flux<String> flow = rxson.create(String.class, req, "$[*].Airport.Name");}
     *
     ...
     */
    public <T> Publisher<T> create(final Class<T> clazz, final HttpRequest req, final String jsonPath) {
        final var subscriber = new MonoReactiveSubscriber<>(clazz, provider, surfer, jsonPath);
        subscriber.setN(n);
        final var bodyHandler =
    ...
    public static final class Builder {
        ...
        private ReactiveProvider provider;
        ...
        /**
         * @param provider ReactiveProvider to be used, if null,
         *                 it will check in the classpath for RxProvider that supports Reactivex io.reactivex.Flowable
         *                 or ReactorProvider that supports Reactor reactor.core.publisher.Flux,
         *                 and if none found, will throw {@link NoReactiveProviderException}
         */
        public Builder provider(final ReactiveProvider provider) {
            this.provider = provider;
            return this;
        }
        ...
        public RxSON build() {
            return new RxSON(
                ...
                Optional.ofNullable(provider).orElse(getDefaultProvider()),
                surfer,
                n);
        }
        ...
        private ReactiveProvider getDefaultProvider() {
            Class<?> clazz;
            try {
                try {
                    clazz = Class.forName("io.rxson.rxjava.RxProvider");
                } catch (ClassNotFoundException ex) {
                    try {
                        clazz = Class.forName("io.rxson.reactor.ReactorProvider");
                    } catch (ClassNotFoundException ex2) {
                        throw new NoReactiveProviderException("No reactive provider found in the class path", ex);
                    }
                }
                final Constructor<?> constructor = clazz.getConstructor();
                return (ReactiveProvider) constructor.newInstance();

            } catch (InvocationTargetException | NoSuchMethodException | IllegalAccessException | InstantiationException e) {
                throw new NoReactiveProviderException("Error initializing the reactive provider", e);
            }
        }
    }
